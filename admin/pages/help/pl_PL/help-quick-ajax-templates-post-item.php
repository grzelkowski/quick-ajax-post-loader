...
<li><p>Dla domyślnego szablonu postów: plik musi być nazwany <code class="code-tag">post-item.php</code> i umieszczony w katalogu <code class="code-tag">/wpg-quick-ajax-post-loader/templates/post-items/</code>. Ten plik automatycznie zastąpi wbudowany domyślny szablon pluginu.</p></li>
<li><p>Dla niestandardowych szablonów: pliki muszą mieć nazwę zaczynającą się od <code class="code-tag">\'post-item\'</code>, np. <code class="code-tag">post-item-custom-name.php</code>. Taki format nazwy pozwala pluginowi na identyfikację plików jako dodatkowych szablonów postów.</p></li>
...
&lt;div class="quick-ajax-post-item"&gt;
    &lt;a href="&lt;?php echo get_permalink() ?&gt;"&gt;
        &lt;!-- Tu dodaj kod wyświetlający post, np. miniaturę, tytuł --&gt;
    &lt;/a&gt;
&lt;/div&gt;
</code></pre>';
$accordion_block_content .= '<h4>Komunikat "Brak postów"</h4>';
$accordion_block_content .= '<p>Możesz również dostosować komunikat wyświetlany, gdy nie ma żadnych postów do wyświetlenia, tworząc plik <code class="code-tag">no-posts.php</code> w katalogu <code class="code-tag">/wpg-quick-ajax-post-loader/templates/</code>, aby nadpisać domyślny komunikat.</p>';
...
$accordion_block_content .= '<ul>
<li><p>Umieszczając szablon w <strong>child theme</strong>, zapewnisz jego trwałość przez aktualizacje motywu głównego.</p></li>
<li><p>Umieszczając szablon bezpośrednio w <strong>theme</strong>, zyskasz możliwość szybkiej personalizacji, ale z ryzykiem utraty zmian przy aktualizacji motywu.</p></li>
<li><p>Plugin automatycznie wykryje dostępne szablony w tych lokalizacjach i umożliwi ich wybór w konfiguracji shortcode.</p></li>
</ul>';
$accordion_block_content .= '<h4>Najlepsze Praktyki</h4>';
$accordion_block_content .= '<ul>
<li><p>Nazwij swoje pliki szablonów w sposób jednoznaczny, aby łatwo je identyfikować.</p></li>
<li><p>Testuj szablony w różnych środowiskach i konfiguracjach, aby upewnić się, że wyglądają dobrze na wszystkich urządzeniach i w różnych przeglądarkach.</p></li>
<li><p>Używaj odpowiednich klas CSS, aby zapewnić spójność stylów z resztą Twojej strony.</p></li>
</ul>';

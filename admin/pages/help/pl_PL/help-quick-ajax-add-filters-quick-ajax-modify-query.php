...
add_filter(\'quick_ajax_modify_query\', function($args, $quick_ajax_id) {
    // Wykorzystanie identyfikatora AJAX do modyfikacji argumentów zapytania
    if ($quick_ajax_id === \'some_specific_id\') {
        $args[\'posts_per_page\'] = 5; // Zmiana liczby postów na stronę na 5
    }
    return $args;
}, 10, 2);
</code></pre>';
...
$accordion_block_content .= '<p><strong>Znajdowanie quick_ajax_id:</strong> Aby znaleźć identyfikator <code class="code-tag">quick_ajax_id</code>, szukaj atrybutu <code class="code-tag">id</code> zewnętrznego diva, który zawiera przyciski AJAX. Na przykład, <code class="code-tag">&lt;div id="quick-ajax-<strong>p9</strong>" class="quick-ajax-posts-wrapper"&gt;</code> wskazuje, że <code class="code-tag">quick_ajax_id</code> to <code class="code-tag">"p9"</code>. Jest to kluczowe dla identyfikacji kontenera w Twoich filtrach.</p>';

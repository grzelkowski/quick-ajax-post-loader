...
final class QAPL_Ajax_Layout_Renderer{
    ...
    public function __construct(QAPL_Quick_Ajax_File_Manager $file_manager, QAPL_Ajax_UI_Renderer $ui_renderer, QAPL_Ajax_Load_More_Renderer $load_more_renderer, QAPL_Ajax_Helper $helper) {
    ...
    public function render_layout($query_args, $source_args, $layout, $attributes, $ajax_initial_load, $quick_ajax_id) {
        ...
        if ($query->have_posts()) {
            ...
            if ($ajax_initial_load) {
                ...
                echo '<div class="qapl-initial-loader" data-button="'.QAPL_Quick_Ajax_Constants::TERM_FILTER_BUTTON_DATA_BUTTON.'" style="display:none;" data-action="' . esc_attr(wp_json_encode($source_args)) . '" data-attributes="' . esc_attr(wp_json_encode($attributes)) . '"></div>';
            } else {
